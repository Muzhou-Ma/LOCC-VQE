...
def train(n: int, init_theta_1_batched: tf.Variable, init_gamma_batched: tf.Variable, CNOT_Ops: list, masks: list,  batch_size: int, kx: float, h: float,  optimizer: tf.keras.optimizers, sample_round: int, max_iter: int, record: int):
    ...
    for iter in range(max_iter):
        ...
        if iter % 500==1:
            np.save(f'./results/Ising_ML/Ising_4_seed={int(rand_seed)}_iter={int(max_iter)}_NNweight=0.01_No_load_Z/energy_batched_record_Z_h={h}iter={iter}.npy', energy_batched_record)
            np.save(f'./results/Ising_ML/Ising_4_seed={int(rand_seed)}_iter={int(max_iter)}_NNweight=0.01_No_load_Z/theta_1_batched_record_Z_h={h}iter={iter}.npy', theta_1_batched_record)
            for i in range(len(gamma_batched_record[-1])):
                np.save(f'./results/Ising_ML/Ising_4_seed={int(rand_seed)}_iter={int(max_iter)}_NNweight=0.01_No_load_Z/gamma_batched_{i}_Z_h={h}_iter={iter}.npy',np.array(gamma_batched_record[-1][i]))
            np.save(f'./results/Ising_ML/Ising_4_seed={int(rand_seed)}_iter={int(max_iter)}_NNweight=0.01_No_load_Z/energy_min_record_Z_h={h}_iter={iter}.npy', energy_min_record)
            plt.figure()
            plt.plot(energy_min_record)
            plt.xlabel('iter')
            plt.ylabel('energy')
            # plt.show()
            plt.savefig(f'./results/Ising_ML/Ising_4_seed={int(rand_seed)}_iter={int(max_iter)}_NNweight=0.01_No_load_Z/Ising_4_Z_h={h}_iter={iter}.jpg')
...
def init_param(n, m, batch_size, random_seed, load_param=False, param_path=None):
    ...
    if load_param:
        ...
        # init_gamma_layer_2_weights = tf.Variable(initial_value=he_init([batch_size, 6*n, 16*m],random_seed[6]))
        # init_gamma_layer_2_bias = tf.Variable(initial_value=tf.random.normal(shape=[batch_size, 6*n, 1], mean=0, stddev=0.1,seed=random_seed[7]))
        ...
        # # np.save('./results/hahahahaha_Reduced1.npy', init_theta_1)
        # # np.save('./results/hahahahaha_Reduced2.npy', init_gamma_6)
        # # np.save('./results/hahahahaha_Reducedgamma_5.npy', init_gamma_5)
...
if __name__ == '__main__':
    ...
    for i in np.arange(0,31):
        ...
        np.save(f'./results/Ising_ML/Ising_4_seed={int(rand_seed)}_iter={int(max_iter)}_NNweight=0.01_No_load_Z/theta_1_batched_Z_h={h}.npy',theta_1_batched_record[-1])
        np.save(f'./results/Ising_ML/Ising_4_seed={int(rand_seed)}_iter={int(max_iter)}_NNweight=0.01_No_load_Z/theta_2_batched_Z_h={h}.npy',theta_2_batched)
        for i in range(len(gamma_batched_record[-1])):
            np.save(f'./results/Ising_ML/Ising_4_seed={int(rand_seed)}_iter={int(max_iter)}_NNweight=0.01_No_load_Z/gamma_batched_{i}_Z_h={h}.npy',np.array(gamma_batched_record[-1][i]))
        
        
        np.save(f'./results/Ising_ML/Ising_4_seed={int(rand_seed)}_iter={int(max_iter)}_NNweight=0.01_No_load_Z/energy_min_record_Z_h={h}.npy', energy_min_record)
        ...
        # plt.show()
        plt.savefig(f'./results/Ising_ML/Ising_4_seed={int(rand_seed)}_iter={int(max_iter)}_NNweight=0.01_No_load_Z/Ising_4_Z_h={h}.jpg')
        
    np.save(f'./results/Ising_ML/Ising_4_seed={int(rand_seed)}_iter={int(max_iter)}_NNweight=0.01_No_load_Z/best_results_Z.npy', best_results)
    plt.figure()
    plt.plot(0.1*np.arange(31), best_results)
    plt.xlabel('h_Z')
    plt.ylabel('energy')
    plt.savefig(f'./results/Ising_ML/Ising_4_seed={int(rand_seed)}_iter={int(max_iter)}_NNweight=0.01_No_load_Z/best_results_Z_new.jpg')
