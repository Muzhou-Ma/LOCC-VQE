...
def Hamiltonian(c: tc.Circuit, h: float = 0):
    ...
    e += (1-h) * c.expectation_ps(x=[0,1,2,3,4,5])
...
def batched_train_step_tf(batch, h, rand_seed, maxiter=1000, random_idx=None, load_param=False, input_param=None):    
    ...
    for i in range(maxiter):
        ...
        # grad_Syn = grad[:,0:20*6] * 1.0 # enhance grad effect for syndrome circuit
        # grad_Corr = grad[:,20*6:744]
        ...
        if i % 100 == 1 and i / 100 > 0:
            np.save(f'./LOCC-VQE_RSC_results/LOCC-VQE_RSC_depth_4_seed=2232119_iter={int(max_iter)}_Z_longstringX/params_Z/Z_h={h}_iter={i}.npy', param)
            
            optim_result = np.min(np.array(energy_lowest))
            optim_results.append(optim_result)
            np.save(f'./LOCC-VQE_RSC_results/LOCC-VQE_RSC_depth_4_seed=2232119_iter={int(max_iter)}_Z_longstringX/results_Z/Z_h={h}_iter={i}.npy',energy_lowest)
            plt.figure()
            plt.plot(energy_lowest)
            plt.xlabel('iter')
            plt.ylabel('energy')
            plt.savefig(f'./LOCC-VQE_RSC_results/LOCC-VQE_RSC_depth_4_seed=2232119_iter={int(max_iter)}_Z_longstringX/results_Z/Z_h={h}_iter={i}.jpg')
...
def GS_energy(h):
    ...
    pauli_list.append(('XXXXXXIIIIII', -(1-h)))
...
if __name__=="__main__":
    ...
    for i in range(6):
        ...
        np.save(f'./LOCC-VQE_RSC_results/LOCC-VQE_RSC_depth_4_seed={int(rand_seed)}_iter={int(max_iter)}_Z_longstringX/params_Z/Z_h={h}.npy', param)
        print(f'i={i},save_path=./LOCC-VQE_RSC_results/LOCC-VQE_RSC_depth_4_seed={int(rand_seed)}_iter={int(max_iter)}_Z_longstringX/params_Z/Z_h={h}.npy')
        
        optim_result = np.min(np.array(energy_lowest))
        optim_results.append(optim_result)
        gap_list.append(optim_result-GS_energy_h)
        np.save(f'./LOCC-VQE_RSC_results/LOCC-VQE_RSC_depth_4_seed={int(rand_seed)}_iter={int(max_iter)}_Z_longstringX/results_Z/Z_h={h}.npy',energy_lowest)
        plt.figure()
        plt.plot(energy_lowest)
        plt.xlabel('iter')
        plt.ylabel('energy')
        plt.title(f'Z_{h}_GS_{GS_energy_h}')
        plt.savefig(f'./LOCC-VQE_RSC_results/LOCC-VQE_RSC_depth_4_seed={int(rand_seed)}_iter={int(max_iter)}_Z_longstringX/results_Z/Z_h={h}.jpg')
        print(f'Ground state energy: {GS_energy_h}')
        print(f'Best optimum result: {optim_result}')
        print(f'gap = {optim_result-GS_energy_h}')
        
    np.save(f'./LOCC-VQE_RSC_results/LOCC-VQE_RSC_depth_4_seed={int(rand_seed)}_iter={int(max_iter)}_Z_longstringX/results_Z/Z_h={h}.npy',np.array(optim_results))
    plt.figure()
    plt.subplot(1,2,1)
