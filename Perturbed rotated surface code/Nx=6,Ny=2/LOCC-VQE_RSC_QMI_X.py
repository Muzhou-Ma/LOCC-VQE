...
if __name__=="__main__":
    ...
    for i in range(11):
        h = i*0.1 + 0.1
        param_path = f'./LOCC-VQE_RSC_results/LOCC-VQE_RSC_depth_4_seed=2232119_iter=600_X/params_X/X_h={h:.1f}_iter=401.npy'
        
        input_param = np.load(param_path)
        print('----------------------------------------------------------------------------')
        print(f'load parameter from {param_path}')
        energy, qmi = batched_train_step_tf(batch,h,input_param)
        print(qmi)
