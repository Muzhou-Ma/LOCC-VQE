...
def toric_syndrome_circuit(theta_Z):
    ...
    for j in range(len(Geo_Z_interaction[1])):
        ...
        # Cartan decompostion
        toric_syndrome.rx(Geo_Z_interaction[1][j][0]+19, theta = theta_Z[theta_idx+j][0])
        toric_syndrome.ry(Geo_Z_interaction[1][j][0]+19, theta = theta_Z[theta_idx+j][1])
        toric_syndrome.rz(Geo_Z_interaction[1][j][0]+19, theta = theta_Z[theta_idx+j][2])
        toric_syndrome.rx(Geo_Z_interaction[1][j][1]+19, theta = theta_Z[theta_idx+j][0])
        toric_syndrome.ry(Geo_Z_interaction[1][j][1]+19, theta = theta_Z[theta_idx+j][1])
        toric_syndrome.rz(Geo_Z_interaction[1][j][1]+19, theta = theta_Z[theta_idx+j][2])
        toric_syndrome.rxx(Geo_Z_interaction[1][j][0]+19,Geo_Z_interaction[1][j][1]+19,theta = theta_Z[theta_idx+j][3])
        toric_syndrome.ryy(Geo_Z_interaction[1][j][0]+19,Geo_Z_interaction[1][j][1]+19,theta = theta_Z[theta_idx+j][4])
        toric_syndrome.rzz(Geo_Z_interaction[1][j][0]+19,Geo_Z_interaction[1][j][1]+19,theta = theta_Z[theta_idx+j][5])
        toric_syndrome.rx(Geo_Z_interaction[1][j][0]+19, theta = theta_Z[theta_idx+j][0])
        toric_syndrome.ry(Geo_Z_interaction[1][j][0]+19, theta = theta_Z[theta_idx+j][1])
        toric_syndrome.rz(Geo_Z_interaction[1][j][0]+19, theta = theta_Z[theta_idx+j][2])
        toric_syndrome.rx(Geo_Z_interaction[1][j][1]+19, theta = theta_Z[theta_idx+j][0])
    ...
    for j in range(len(Geo_Z_interaction[2])):
        ...
        # Cartan decompostion
        toric_syndrome.rx(Geo_Z_interaction[2][j][0]+19, theta = theta_Z[theta_idx+j][0])
        toric_syndrome.ry(Geo_Z_interaction[2][j][0]+19, theta = theta_Z[theta_idx+j][1])
        toric_syndrome.rz(Geo_Z_interaction[2][j][0]+19, theta = theta_Z[theta_idx+j][2])
        toric_syndrome.rx(Geo_Z_interaction[2][j][1]+19, theta = theta_Z[theta_idx+j][0])
        toric_syndrome.ry(Geo_Z_interaction[2][j][1]+19, theta = theta_Z[theta_idx+j][1])
        toric_syndrome.rz(Geo_Z_interaction[2][j][1]+19, theta = theta_Z[theta_idx+j][2])
        toric_syndrome.rxx(Geo_Z_interaction[2][j][0]+19,Geo_Z_interaction[2][j][1]+19,theta = theta_Z[theta_idx+j][3])
        toric_syndrome.ryy(Geo_Z_interaction[2][j][0]+19,Geo_Z_interaction[2][j][1]+19,theta = theta_Z[theta_idx+j][4])
        toric_syndrome.rzz(Geo_Z_interaction[2][j][0]+19,Geo_Z_interaction[2][j][1]+19,theta = theta_Z[theta_idx+j][5])
        toric_syndrome.rx(Geo_Z_interaction[2][j][0]+19, theta = theta_Z[theta_idx+j][0])
        toric_syndrome.ry(Geo_Z_interaction[2][j][0]+19, theta = theta_Z[theta_idx+j][1])
        toric_syndrome.rz(Geo_Z_interaction[2][j][0]+19, theta = theta_Z[theta_idx+j][2])
        toric_syndrome.rx(Geo_Z_interaction[2][j][1]+19, theta = theta_Z[theta_idx+j][0])
    ...
    for i in range(2):
        for j in range(len(Geo_Z_interaction[i+4])):
            ...
            # Cartan decompostion
            toric_syndrome.rx(Geo_Z_interaction[i+4][j][0]+19, theta = theta_Z[theta_idx+j][0])
            toric_syndrome.ry(Geo_Z_interaction[i+4][j][0]+19, theta = theta_Z[theta_idx+j][1])
            toric_syndrome.rz(Geo_Z_interaction[i+4][j][0]+19, theta = theta_Z[theta_idx+j][2])
            toric_syndrome.rx(Geo_Z_interaction[i+4][j][1]+19, theta = theta_Z[theta_idx+j][0])
            toric_syndrome.ry(Geo_Z_interaction[i+4][j][1]+19, theta = theta_Z[theta_idx+j][1])
            toric_syndrome.rz(Geo_Z_interaction[i+4][j][1]+19, theta = theta_Z[theta_idx+j][2])
            toric_syndrome.rxx(Geo_Z_interaction[i+4][j][0]+19,Geo_Z_interaction[i+4][j][1]+19,theta = theta_Z[theta_idx+j][3])
            toric_syndrome.ryy(Geo_Z_interaction[i+4][j][0]+19,Geo_Z_interaction[i+4][j][1]+19,theta = theta_Z[theta_idx+j][4])
            toric_syndrome.rzz(Geo_Z_interaction[i+4][j][0]+19,Geo_Z_interaction[i+4][j][1]+19,theta = theta_Z[theta_idx+j][5])
            toric_syndrome.rx(Geo_Z_interaction[i+4][j][0]+19, theta = theta_Z[theta_idx+j][0])
            toric_syndrome.ry(Geo_Z_interaction[i+4][j][0]+19, theta = theta_Z[theta_idx+j][1])
            toric_syndrome.rz(Geo_Z_interaction[i+4][j][0]+19, theta = theta_Z[theta_idx+j][2])
            toric_syndrome.rx(Geo_Z_interaction[i+4][j][1]+19, theta = theta_Z[theta_idx+j][0])
    ...
    for j in range(len(Geo_Z_interaction[i+4])):
        ...
        # Cartan decompostion
        toric_syndrome.rx(Geo_Z_interaction[6][j][0]+19, theta = theta_Z[theta_idx+j][0])
        toric_syndrome.ry(Geo_Z_interaction[6][j][0]+19, theta = theta_Z[theta_idx+j][1])
        toric_syndrome.rz(Geo_Z_interaction[6][j][0]+19, theta = theta_Z[theta_idx+j][2])
        toric_syndrome.rx(Geo_Z_interaction[6][j][1]+19, theta = theta_Z[theta_idx+j][0])
        toric_syndrome.ry(Geo_Z_interaction[6][j][1]+19, theta = theta_Z[theta_idx+j][1])
        toric_syndrome.rz(Geo_Z_interaction[6][j][1]+19, theta = theta_Z[theta_idx+j][2])
        toric_syndrome.rxx(Geo_Z_interaction[6][j][0]+19,Geo_Z_interaction[6][j][1],theta = theta_Z[theta_idx+j][3])
        toric_syndrome.ryy(Geo_Z_interaction[6][j][0]+19,Geo_Z_interaction[6][j][1],theta = theta_Z[theta_idx+j][4])
        toric_syndrome.rzz(Geo_Z_interaction[6][j][0]+19,Geo_Z_interaction[6][j][1],theta = theta_Z[theta_idx+j][5])
        toric_syndrome.rx(Geo_Z_interaction[6][j][0]+19, theta = theta_Z[theta_idx+j][0])
        toric_syndrome.ry(Geo_Z_interaction[6][j][0]+19, theta = theta_Z[theta_idx+j][1])
        toric_syndrome.rz(Geo_Z_interaction[6][j][0]+19, theta = theta_Z[theta_idx+j][2])
        toric_syndrome.rx(Geo_Z_interaction[6][j][1]+19, theta = theta_Z[theta_idx+j][0])
    ...
    for i in range(2):
        for j in range(len(Geo_X_interaction[i])):
            toric_syndrome.rx(Geo_X_interaction[i][j][0]+19, theta = theta_Z[theta_idx+j][0]) # although we still call it theta_Z, it is actually parameters for X stabilizers
            toric_syndrome.ry(Geo_X_interaction[i][j][0]+19, theta = theta_Z[theta_idx+j][1])
            toric_syndrome.rz(Geo_X_interaction[i][j][0]+19, theta = theta_Z[theta_idx+j][2])
            toric_syndrome.rx(Geo_X_interaction[i][j][1]+19, theta = theta_Z[theta_idx+j][0])
            toric_syndrome.ry(Geo_X_interaction[i][j][1]+19, theta = theta_Z[theta_idx+j][1])
            toric_syndrome.rz(Geo_X_interaction[i][j][1]+19, theta = theta_Z[theta_idx+j][2])
            toric_syndrome.rxx(Geo_X_interaction[i][j][0]+19,Geo_X_interaction[i][j][1],theta = theta_Z[theta_idx+j][3])
            toric_syndrome.ryy(Geo_X_interaction[i][j][0]+19,Geo_X_interaction[i][j][1],theta = theta_Z[theta_idx+j][4])
            toric_syndrome.rzz(Geo_X_interaction[i][j][0]+19,Geo_X_interaction[i][j][1],theta = theta_Z[theta_idx+j][5])
            toric_syndrome.rx(Geo_X_interaction[i][j][0]+19, theta = theta_Z[theta_idx+j][0])
            toric_syndrome.ry(Geo_X_interaction[i][j][0]+19, theta = theta_Z[theta_idx+j][1])
            toric_syndrome.rz(Geo_X_interaction[i][j][0]+19, theta = theta_Z[theta_idx+j][2])
            toric_syndrome.rx(Geo_X_interaction[i][j][1]+19, theta = theta_Z[theta_idx+j][0])
            toric_syndrome.ry(Geo_X_interaction[i][j][1]+19, theta = theta_Z[theta_idx+j][1])
...
def correction_circuit(params_corr_1, params_corr_2, params_corr_3):
    ...
    for i in range(7):
        for j in range(12):
            corr_circuit.ry(12+i+19,theta=np.pi/2)
            corr_circuit.rxx(12+i+19,j+19,theta=params_corr_1[i][j][0])
            corr_circuit.rx(j+19,theta=params_corr_1[i][j][1])
            corr_circuit.ry(12+i+19,theta=-np.pi/2)
    
    for i in range(7):
        for j in range(12):
            corr_circuit.ry(12+i,theta=np.pi/2)
            corr_circuit.rxx(12+i,j+19,theta=params_corr_1[i][j][0])
            corr_circuit.rx(j+19,theta=params_corr_1[i][j][1])
            corr_circuit.ry(12+i,theta=-np.pi/2)
    ...
    for i in range(7):
        for j in range(12):
            corr_circuit.ry(12+i+19,theta=np.pi/2)
            corr_circuit.rxx(12+i+19,j,theta=params_corr_1[i][j][0])
            corr_circuit.rx(j,theta=params_corr_1[i][j][1])
            corr_circuit.ry(12+i+19,theta=-np.pi/2)
...
def Hamiltonian(c: tc.Circuit, h: float = 0):
    ...
    e += (1-h) * c.expectation_ps(x=[0+19, 6+19])
    e += (1-h) * c.expectation_ps(x=[1+19, 2+19, 7+19, 8+19])
    e += (1-h) * c.expectation_ps(x=[3+19, 4+19, 9+19, 10+19]) # this
    e += (1-h) * c.expectation_ps(x=[5, 11])
    
    e += (1-h) * c.expectation_ps(z=[0+19, 1+19, 6+19, 7+19])
    e += (1-h) * c.expectation_ps(z=[1+19, 2+19])
    e += (1-h) * c.expectation_ps(z=[7+19, 8+19])
    e += (1-h) * c.expectation_ps(z=[2+19, 3+19, 8+19, 9+19])
    e += (1-h) * c.expectation_ps(z=[3+19, 4+19])
    e += (1-h) * c.expectation_ps(z=[9+19, 10+19])
    e += (1-h) * c.expectation_ps(z=[4+19, 5+19, 10+19, 11+19])
    
    for i in range(12):
        e += h * c.expectation_ps(x=[i])
...
def batched_train_step_tf(batch, h, rand_seed, maxiter=1000, random_idx=None, load_param=False, input_param=None):    
    ...
    for i in range(maxiter):
        ...
        if i % 100 == 1 and i / 100 > 0:
            np.save(f'./LOCC-VQE_RSC_results/LOCC-VQE_RSC_depth_4_seed=2232119_iter={int(max_iter)}_X/params_X/X_h={h}_iter={i}.npy', param)
            
            optim_result = np.min(np.array(energy_lowest))
            optim_results.append(optim_result)
            np.save(f'./LOCC-VQE_RSC_results/LOCC-VQE_RSC_depth_4_seed=2232119_iter={int(max_iter)}_X/results_X/X_h={h}_iter={i}.npy',energy_lowest)
            plt.figure()
            plt.plot(energy_lowest)
            plt.xlabel('iter')
            plt.ylabel('energy')
            plt.savefig(f'./LOCC-VQE_RSC_results/LOCC-VQE_RSC_depth_4_seed=2232119_iter={int(max_iter)}_X/results_X/X_h={h}_iter={i}.jpg')
...
if __name__=="__main__":
    ...
    for i in range(11):
        ...
        np.save(f'./LOCC-VQE_RSC_results/LOCC-VQE_RSC_depth_4_seed={int(rand_seed)}_iter={int(max_iter)}_X/params_X/X_h={h}.npy', param)
        print(f'i={i},save_path=./LOCC-VQE_RSC_results/LOCC-VQE_RSC_depth_4_seed={int(rand_seed)}_iter={int(max_iter)}_X/params_X/X_h={h}.npy')
        
        optim_result = np.min(np.array(energy_lowest))
        optim_results.append(optim_result)
        gap_list.append(optim_result-GS_energy_h)
        np.save(f'./LOCC-VQE_RSC_results/LOCC-VQE_RSC_depth_4_seed={int(rand_seed)}_iter={int(max_iter)}_X/results_X/X_h={h}.npy',energy_lowest)
        plt.figure()
        plt.plot(energy_lowest)
        plt.xlabel('iter')
        plt.ylabel('energy')
        plt.title(f'X_{h}_GS_{GS_energy_h}')
        plt.savefig(f'./LOCC-VQE_RSC_results/LOCC-VQE_RSC_depth_4_seed={int(rand_seed)}_iter={int(max_iter)}_X/results_X/X_h={h}.jpg')
        print(f'Ground state energy: {GS_energy_h}')
        print(f'Best optimum result: {optim_result}')
        print(f'gap = {optim_result-GS_energy_h}')
        
    np.save(f'./LOCC-VQE_RSC_results/LOCC-VQE_RSC_depth_4_seed={int(rand_seed)}_iter={int(max_iter)}_X/results_X/X_h={h}.npy',np.array(optim_results))
    plt.figure()
    plt.subplot(1,2,1)
